...
# ============================================================
# /realtime resource — GET /realtime → realtime Lambda
# ============================================================
...
resource "aws_api_gateway_integration" "realtime_get" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_alias.realtime_live.invoke_arn
}
...
resource "aws_api_gateway_method_response" "realtime_options_200" {
  rest_api_id = aws_api_gateway_rest_api.cloudpulse.id
  resource_id = aws_api_gateway_resource.realtime.id
  http_method = aws_api_gateway_method.realtime_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "realtime_options" {
  ...
  resource_id = aws_api_gateway_resource.realtime.id
  http_method = aws_api_gateway_method.realtime_options.http_method
  status_code = aws_api_gateway_method_response.realtime_options_200.status_code
  response_parameters = {
    "method.response.header.Access-Control-Allow-Headers" = "'Content-Type,Authorization'"
    "method.response.header.Access-Control-Allow-Methods" = "'GET,OPTIONS'"
    "method.response.header.Access-Control-Allow-Origin"  = "'*'"
...
resource "aws_api_gateway_deployment" "cloudpulse" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.realtime_get.id,
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.realtime_get,
    ...
    aws_api_gateway_integration.realtime_options,
  ]
}
